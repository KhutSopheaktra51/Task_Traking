...
foreach (['todo', 'in_progress', 'in_review', 'blocked', 'done'] as $s) {
    ...
    $task_by_status[$s] = (int) $row['total'];
}
...
foreach (['low', 'medium', 'high', 'urgent'] as $p) {
    ...
    $task_by_priority[$p] = (int) $row['total'];
}
...
<!-- Month filter -->
<form method="GET" action="./" class="mb-6 flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-4">
    <input type="hidden" name="page" value="report/index">
    <label class="text-sm font-medium text-gray-700">Month</label>
    <input type="month" name="month" value="<?= e($month) ?>" class="px-3 py-2 border border-gray-300 rounded-lg text-sm
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100">
        <h2 class="text-sm font-semibold text-gray-700">
            Attendance summary —
            <?= date('F Y', mktime(0, 0, 0, (int) $mon, 1, (int) $year)) ?>
        </h2>
    </div>
    ...
    new Chart(document.getElementById('status_chart').getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{
                data: [
                    <?= $task_by_status['todo'] ?>,
                    <?= $task_by_status['in_progress'] ?>,
                    <?= $task_by_status['in_review'] ?>,
                    <?= $task_by_status['blocked'] ?>,
                    <?= $task_by_status['done'] ?>
                ],
    ...
    new Chart(document.getElementById('priority_chart').getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{
                label: 'Tasks',
                data: [
                    <?= $task_by_priority['low'] ?>,
                    <?= $task_by_priority['medium'] ?>,
                    <?= $task_by_priority['high'] ?>,
                    <?= $task_by_priority['urgent'] ?>
                ],
