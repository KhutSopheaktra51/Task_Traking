...
<form method="GET" action="./" class="flex flex-wrap gap-3 mb-6 bg-white rounded-xl border border-gray-200 p-4">
    ...
    <input type="text" name="search" value="<?= e($_GET['search'] ?? '') ?>" placeholder="Search tasks..." class="flex-1 min-w-40 px-3 py-2 text-sm border border-gray-300 rounded-lg
                  focus:outline-none focus:ring-2 focus:ring-indigo-500">
    <select name="priority" class="px-3 py-2 text-sm border border-gray-300 rounded-lg
                   focus:outline-none focus:ring-2 focus:ring-indigo-500">
    ...
    <?php if (isManagerOrAdmin()): ?>
        <select name="assignee_id" class="px-3 py-2 text-sm border border-gray-300 rounded-lg
                   focus:outline-none focus:ring-2 focus:ring-indigo-500">
...
<div x-data="kanban()" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
    ...
    foreach ($columns as $status => $col_tasks):
        ?>
            ...
            <div class="flex-1 p-3 space-y-2 min-h-24">
                ...
                <?php foreach ($col_tasks as $task): ?>
                    <div class="bg-white rounded-lg border border-gray-200 p-3 cursor-grab
                        hover:border-indigo-300 transition-colors" draggable="true"
        ...
        return {
            ...
            drop(event, new_status) {
                ...
                fetch('./?page=task/update_status', {
                    ...
                    body: 'id=' + this.dragging_id + '&status=' + new_status + '&csrf_token=<?= csrfToken() ?>',
