...
<div class="max-w-2xl">
    ...
    <form method="POST" action="./?page=task/edit&id=<?= $id ?>"
        ...
        <div>
            ...
            <input type="text" name="title" value="<?= e($task['title']) ?>" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                             focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"><?= e($task['description']) ?></textarea>
        </div>
            ...
            <div>
                ...
                <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                    ...
                    <?php foreach (['todo' => 'To Do', 'in_progress' => 'In Progress', 'in_review' => 'In Review', 'blocked' => 'Blocked', 'done' => 'Done'] as $v => $l): ?>
                        <option value="<?= $v ?>" <?= ($task['status'] === $v) ? 'selected' : '' ?>>
                            <?= $l ?>
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="due_date" value="<?= e($task['due_date']) ?>" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm
                              focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
        </div>
